...
def besd2txt(fi):
    ...
    try:
        ...
        output_file = fi.replace('.besd', '.txt')
        
        cmd = [
            osca_bin,
            '--beqtl-summary', fi.replace(".besd", ""),
            '--query', '1',
            '--out', output_file
        ]
        ...
        # Compress the output
        compress_cmd = ['gzip', output_file]
        logger.info(f"Compressing output: {' '.join(compress_cmd)}")
